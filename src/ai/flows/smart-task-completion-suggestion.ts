...
const smartTaskCompletionSuggestionPrompt = ai.definePrompt({
  ...
  prompt: `You are an AI assistant tasked with suggesting whether a given 'currentTask' should be marked as complete. Evaluate the 'currentTask' considering its details (title, description, due date, priority, category) and the context provided by 'allTasks'.
...
{{#each allTasks}}
- Title: {{{this.title}}}{{#if this.description}} - Description: {{{this.description}}}{{/if}}{{#if this.dueDate}} - Due Date: {{{this.dueDate}}}{{/if}}{{#if this.priority}} - Priority: {{{this.priority}}}{{/if}}{{#if this.category}} - Category: {{{this.category}}}{{/if}} - Is Complete: {{{this.isComplete}}}
{{/each}}
